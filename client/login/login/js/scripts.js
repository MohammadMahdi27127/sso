...
async function save(phone) { // دریافت شماره موبایل به عنوان پارامتر
    const connection = await mysql.createConnection(connectionConfig);
    try {
    ...
    if (phonePattern.test(phone)) {
        ...
        $.ajax({
            ...
            success: function(response) {
                // بررسی پاسخ موفقیت‌آمیز از API
                if (response) { // بررسی اینکه آیا پاسخ معتبر است
                    $(".next").click(function () {
...
function verifyCode() {
    ...
    $.ajax({
        url: 'http://127.0.0.1:8080/api/create_mobile/', // آدرس API تأیید خود را اینجا قرار دهید
        type: 'POST',
        data: { code: codeInput },
        success: function(response) {
            if (response.success) {
